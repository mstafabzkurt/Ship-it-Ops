      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.rankList}>
          {RANKS.map((rank) => {
            ...
            return (
              ...
              >
                <View style={[styles.rankDot, reached && { backgroundColor: cfg.color }]} />
                <Text style={styles.rankEmoji}>{cfg.emoji}</Text>
                <View style={styles.rankTextWrap}>
                  <Text style={[styles.rankName, isCurrent && { color: cfg.color }]}>{rank.name}</Text>
        ...
        <View style={styles.badgeSummaryRow}>
          <View style={styles.badgeSummaryChip}>
            ...
            <Text style={styles.badgeSummaryText}>{badges.filter((b) => b.earned).length} kazanıldı</Text>
          </View>
...
function makeStyles(theme: Theme) {
  ...
  return StyleSheet.create({
    ...
    header: {
      fontFamily: fonts.headingSemiBold, fontSize: fontSizes['3xl'], color: colors.textPrimary,
      marginHorizontal: 20, marginTop: 18, marginBottom: 4,
    },
    progressCard: {
      marginHorizontal: 20, marginTop: 14, backgroundColor: colors.panel,
      borderWidth: geometry.borderWidth, borderColor: colors.border,
    ...
    rankList: { marginHorizontal: 20, gap: 6 },
    rankRow: {
      flexDirection: 'row', alignItems: 'center', gap: 10,
      backgroundColor: colors.panel, borderWidth: geometry.borderWidth, borderColor: colors.border,
      borderRadius: geometry.borderRadius, paddingVertical: 9, paddingHorizontal: 12,
    },
    rankRowLocked: { opacity: 0.4 },
    rankDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: colors.border },
    rankEmoji: { fontSize: 14 },
    rankTextWrap: { flex: 1 },
    rankName: { fontFamily: fonts.bodySemiBold, fontSize: fontSizes.sm, color: colors.textPrimary },
    rankThreshold: { fontFamily: fonts.mono, fontSize: fontSizes.xs, color: colors.textMuted, marginTop: 1 },
    currentTagWrap: { paddingHorizontal: 7, paddingVertical: 2, borderRadius: geometry.borderRadiusSm, borderWidth: geometry.borderWidth },
    currentTag: { fontFamily: fonts.bodySemiBold, fontSize: fontSizes.xs, textTransform: 'uppercase' },
    reachedCheck: { fontFamily: fonts.bodySemiBold, fontSize: fontSizes.md, color: colors.accentPositive },
    badgeSummaryRow: { flexDirection: 'row', marginHorizontal: 20, marginBottom: 10, gap: 8 },
    badgeSummaryChip: {
      flexDirection: 'row', alignItems: 'center', gap: 6,
      backgroundColor: colors.positiveBg, borderWidth: geometry.borderWidth,
      borderColor: colors.positiveBorder, borderRadius: geometry.borderRadiusSm,
      paddingVertical: 5, paddingHorizontal: 10, ...effects.glowPositive,
    },
    badgeSummaryChipLocked: { backgroundColor: colors.panelAlt, borderColor: colors.border, ...({} as object) },
    badgeSummaryIcon: { fontSize: 13 },
    badgeSummaryText: { fontFamily: fonts.bodySemiBold, fontSize: fontSizes.xs, color: colors.accentPositive },
    badgeSummaryTextLocked: { color: colors.textMuted },
    badgeGrid: { flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: 20, gap: 10 },
    badgeGridItem: { flexBasis: '47%', flexGrow: 1 },
  });
}
