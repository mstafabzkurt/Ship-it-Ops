...
export default function RoomStoreScreen() {
  ...
  let buttonLabel = '';
  if (isMaxLevel) buttonLabel = '✓ Maksimum Seviye';
  else if (!canAfford) buttonLabel = 'Yetersiz Bütçe';
  else if (nextPrice === 0) buttonLabel = '🏗️ Ücretsiz Yükselt';
  else buttonLabel = `⬆️ $${nextPrice.toLocaleString('tr-TR')} ile Yükselt`;
          ...
          <View style={styles.cardBody}>
            <View style={styles.titleRow}>
              ...
              {isMaxLevel ? (
                ...
                <View style={styles.levelBadge}><Text style={styles.levelBadgeText}>Lv {roomLevel}</Text></View>
              )}
            </View>
            ...
            >
              <Text style={[styles.buttonText, isMaxLevel && styles.buttonTextMax, !canAfford && !isMaxLevel && styles.buttonTextDisabled]}>
                {buttonLabel}
              </Text>
            </TouchableOpacity>
          </View>
...
function makeStyles(theme: Theme) {
  ...
  return StyleSheet.create({
    ...
    card: {
      ...
      borderRadius: geometry.borderRadius, padding: 14, flexDirection: 'row', gap: 14, ...effects.cardShadow,
    },
    cardMax: { borderColor: colors.alertBorder, backgroundColor: colors.alertBg, ...effects.glowAlert },
    cardIconWrap: {
      width: 52, height: 52, borderRadius: geometry.borderRadius, backgroundColor: colors.panelAlt,
      alignItems: 'center', justifyContent: 'center', borderWidth: geometry.borderWidth, borderColor: colors.border, flexShrink: 0,
    },
    ...
    maxBadge: { backgroundColor: colors.alertBg, borderWidth: geometry.borderWidth, borderColor: colors.alertBorder, paddingHorizontal: 6, paddingVertical: 2, borderRadius: geometry.borderRadiusSm },
    maxBadgeText: { fontFamily: fonts.bodySemiBold, fontSize: fontSizes.xs, color: colors.accentAlert, letterSpacing: 0.5 },
    levelBadge: { backgroundColor: colors.positiveBg, borderWidth: geometry.borderWidth, borderColor: colors.positiveBorder, paddingHorizontal: 6, paddingVertical: 2, borderRadius: geometry.borderRadiusSm },
    ...
    nextRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.panelAlt, borderRadius: geometry.borderRadiusSm, paddingHorizontal: 10, paddingVertical: 6, gap: 8 },
    nextLabel: { fontFamily: fonts.body, fontSize: fontSizes.xs, color: colors.textMuted, flex: 1 },
    priceTag: { backgroundColor: colors.positiveBg, borderWidth: geometry.borderWidth, borderColor: colors.positiveBorder, paddingHorizontal: 7, paddingVertical: 3, borderRadius: geometry.borderRadiusSm },
    ...
    freeTag: { backgroundColor: colors.alertBg, borderWidth: geometry.borderWidth, borderColor: colors.alertBorder, paddingHorizontal: 7, paddingVertical: 3, borderRadius: geometry.borderRadiusSm },
    ...
    button: { borderRadius: geometry.borderRadiusSm, paddingVertical: 9, paddingHorizontal: 12, alignItems: 'center', justifyContent: 'center', marginTop: 6 },
    buttonActive: { backgroundColor: colors.accentPositive, ...effects.glowPositive },
    buttonMax: { backgroundColor: colors.alertBg, borderWidth: geometry.borderWidth, borderColor: colors.alertBorder },
    buttonDisabled: { backgroundColor: colors.panelAlt, borderWidth: geometry.borderWidth, borderColor: colors.border },
